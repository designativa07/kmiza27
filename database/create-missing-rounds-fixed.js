...
async function createMissingRounds() {
  ...
  try {
    ...
    // Verificar estrutura da tabela rounds
    ...
    console.log('📋 Colunas da tabela rounds:', tableStructure.rows.map(r => r.column_name).join(', '));
    ...
    for (let roundNum = 1; roundNum <= 38; roundNum++) {
      if (!existingRoundNumbers.includes(roundNum)) {
        try {
          await client.query(`
            INSERT INTO rounds (name, competition_id)
            VALUES ($1, $2)
