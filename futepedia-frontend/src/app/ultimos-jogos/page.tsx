...
const UltimosJogosPage: NextPage = () => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/matches?fromDate=${fromDate}&toDate=${today}&status=finished`);
        if (!response.ok) {
          throw new Error('Erro ao buscar jogos dos últimos 3 dias');
        }
        const data = await response.json();
        setMatches(data.data || data);
      } catch (err) {
        console.error('Erro ao buscar jogos dos últimos 3 dias:', err);
        setError('Erro ao carregar jogos dos últimos 3 dias');
      } finally {
        setLoading(false);
      }
    };
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-white rounded-lg shadow-sm">
            <div className="p-4 border-b border-gray-100">
              <h1 className="text-xl font-bold text-gray-900">Últimos Jogos</h1>
            </div>
            <div className="p-4">
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-4 border-b border-gray-100">
            <h1 className="text-xl font-bold text-gray-900 mb-4">Últimos Jogos</h1>
          ...
          <div className="p-4">
            {matches.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-600 text-lg">Não há jogos finalizados nos últimos 3 dias.</p>
...
export default UltimosJogosPage; 