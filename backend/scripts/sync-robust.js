...
/**
 * Script de sincronização robusto que lida com:
 * - Chaves duplicadas (upsert)
 * - Chaves estrangeiras (validação prévia)
 * - Dados órfãos (filtragem)
 */

async function syncRobust() {
  console.log('🔄 Sincronização Robusta - Produção → Desenvolvimento');
  console.log('====================================================');
  ...
  try {
    ...
    const tablesToClean = [
      ...
      'whatsapp_menu_configs', 'simulation_results', 'system_settings', 'bot_configs'
    ];
    ...
    for (const tableName of syncOrder) {
      try {
        ...
        let validRows = sourceData.rows;
        
        if (tableName === 'teams') {
          // Verificar se stadiums existem
          const stadiumIds = await devClient.query('SELECT id FROM stadiums');
          const validStadiumIds = new Set(stadiumIds.rows.map(row => row.id));
          
          validRows = sourceData.rows.filter(row => {
            if (row.stadium_id && !validStadiumIds.has(row.stadium_id)) {
              console.log(`⚠️  Removendo team ${row.name} - stadium_id ${row.stadium_id} não existe`);
              return false;
            }
            return true;
          });
        ...
        if (tableName === 'competition_teams') {
          ...
          validRows = sourceData.rows.filter(row => {
            if (!validTeamIds.has(row.team_id)) {
              console.log(`⚠️  Removendo competition_team - team_id ${row.team_id} não existe`);
              return false;
            }
            return true;
          });
        ...
        if (tableName === 'users') {
          ...
          validRows = sourceData.rows.map(row => {
            if (row.favorite_team_id && !validTeamIds.has(row.favorite_team_id)) {
              console.log(`⚠️  Definindo favorite_team_id como null para user ${row.name}`);
        ...
        if (validRows.length === 0) {
          console.log(`⚠️  Nenhum registro válido para ${tableName} após filtros`);
          continue;
        }
