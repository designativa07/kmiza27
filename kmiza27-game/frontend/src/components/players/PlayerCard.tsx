...
export default function PlayerCard({ player }: Props) {
  ...
  const badge = (
    <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold" style={{ backgroundColor: '#1e88e5' }}>
      {label}
    </div>
  );
  ...
  const Bar = ({ label, value }: { label: string; value: number }) => (
    <div className="flex items-center gap-2">
      <span className="w-8 text-[10px] tracking-wide text-gray-500">{label}</span>
      <div className="flex-1 h-1.5 bg-gray-100 rounded">
        <div className="h-1.5 rounded bg-blue-500" style={{ width: `${Math.max(0, Math.min(100, value))}%` }} />
      </div>
      <span className="w-6 text-[10px] text-gray-600 text-right">{value}</span>
    </div>
  );
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[12px] text-gray-600">{player.age ?? '—'} anos</div>
          </div>
        </div>
        <div className="text-right">
          <div className="text-lg font-bold text-blue-600">{player.overall ?? '-'}</div>
          <div className="text-[12px] text-gray-600">Overall</div>
          {player.salary != null && (
            <div className="text-[12px] text-emerald-700 font-medium mt-1">R$ {player.salary.toLocaleString()}/mês</div>
          )}
        </div>
      </div>
