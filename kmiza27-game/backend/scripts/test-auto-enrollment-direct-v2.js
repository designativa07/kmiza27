...
async function testAutoEnrollmentDirectV2() {
  console.log('🔍 Testando autoEnrollInCompetition diretamente (v2)...\n');
  ...
  try {
    ...
    // 2. Simular o método autoEnrollInCompetition com a nova lógica
    console.log('\n2. Simulando autoEnrollInCompetition (ordem decrescente)...');
    
    // Buscar competições disponíveis (ordem decrescente - Série D primeiro)
    console.log('   - Buscando competições disponíveis...');
    const { data: competitions, error: compError } = await supabase
      .from('game_competitions')
      .select('id, name, tier, current_teams, max_teams')
      .eq('status', 'active')
      .order('tier', { ascending: false }); // Priorizar Série D (tier 4) primeiro
    ...
    // Escolher a primeira competição disponível (agora será Série D primeiro)
