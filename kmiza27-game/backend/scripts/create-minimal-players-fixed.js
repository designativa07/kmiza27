...
async function createMinimalPlayersFixed() {
  ...
  try {
    ...
    // Contar jogadores por time manualmente
    const playersPerTeam = {};
    allPlayers?.forEach(player => {
      playersPerTeam[player.team_id] = (playersPerTeam[player.team_id] || 0) + 1;
    });

    console.log(`📊 Total de jogadores existentes: ${allPlayers?.length || 0}`);
